...
### Gotchas
* Interactive `gh` goes through the 1Password plugin alias. Non-interactive shells get `GH_TOKEN` (the `omair-inam` account) from `.envrc`, which reads it with a service account token kept in the Keychain. Git ignores `GH_TOKEN`; push over HTTPS with `git -c credential.helper= -c 'credential.helper=!gh auth git-credential' push`
* Git commits are SSH-signed through 1Password's agent. `Couldn't find key in agent?` means `SSH_AUTH_SOCK` points at the empty launchd agent, not that 1Password is locked; set it to `$HOME/Library/Group Containers/2BUA8C4S2C.com.1password/t/agent.sock`
